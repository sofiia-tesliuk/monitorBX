...
int main(int argc, char **argv) {
    ...
    err = bpf_prog_load(bpf_file, BPF_PROG_TYPE_XDP, &obj, &prog_fd);
    if (err < 0) {
        ...
        printf("err: %d, prog_fd: %d", err, prog_fd); // Noticed that err (that supposed to be fd of bpf program and prog_fd are different.)
    }
    ...
    for(int i = 0; i < 100; i++){
        ...
        while (bpf_map_get_next_key(map_fd, &key, &next_key) == 0){
            ...
            print_ip_info(df, next_key, value);
            key = next_key;
        }
